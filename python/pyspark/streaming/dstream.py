...
class DStream(object):
    ...
    def count(self):
        ...
        """
        return self.mapPartitions(lambda i: [sum(1 for _ in i)]).reduce(
            operator.add)
    ...
    def addChildInDag(self, child):
        ...
        childInfo["uid"] = hashlib.sha224(
            child.operation + (str(child.serializableFunction)).encode(
                'utf-8') + childInfo["parent"]).hexdigest()
    ...
    def reduce(self, func):
        ...
        """
        return self.map(lambda x: (None, x)).reduceByKey(func, 1).map(
            lambda x: x[1])
    ...
    def combineByKey(self, createCombiner, mergeValue, mergeCombiners,
        ...
        def func(rdd):
            return rdd.combineByKey(createCombiner, mergeValue, mergeCombiners,
                                    numPartitions)
    ...
    def partitionBy(self, numPartitions, partitionFunc=portable_hash):
        ...
        """
        return self.transform(
            lambda rdd: rdd.partitionBy(numPartitions, partitionFunc))
        ...
        def reduceFunc(t, a, b):
            if a is None:
                g = b.groupByKey(numPartitions).mapValues(
                    lambda vs: (list(vs), None))
            else:
                g = a.cogroup(b.partitionBy(numPartitions), numPartitions)
                g = g.mapValues(lambda ab: (
                list(ab[1]), list(ab[0])[0] if len(ab[0]) else None))
